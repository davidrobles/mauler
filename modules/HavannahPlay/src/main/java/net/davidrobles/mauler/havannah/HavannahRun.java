...
public class HavannahRun {
    ...
    public static void runGUI() {
        ...
        List<Strategy<Havannah>> players =
                new ArrayList<Strategy<Havannah>>() {
                    {
        ...
        MatchController<Havannah> mc =
                new MatchController<Havannah>(() -> new Havannah(8), players, 50);
        MatchControllerButtonsView<Havannah> buttonsView =
                new MatchControllerButtonsView<Havannah>(mc);
        MatchControllerSliderView<Havannah> sliderView =
                new MatchControllerSliderView<Havannah>(mc);
        TableModel model = new HavannahTableModel(mc);
        MatchControllerTableView<Havannah> mcTableView =
                new MatchControllerTableView<Havannah>(model, mc);
        mc.registerObserver(mcTableView);
        mc.registerObserver(buttonsView);
        mc.registerObserver(sliderView);
        mc.registerObserver(panel);
        BoardApp<Havannah> boardApp =
                new BoardApp<Havannah>(panel, mc, buttonsView, sliderView, mcTableView);
