...
/**
 * Facilitates checks for common conditions. Methods in this class throw appropriate exceptions when
...
public class Check {
    ...
    public static void state(boolean condition, String message) {
        if (!condition) throw new IllegalStateException("required condition: " + message);
    }
    ...
    public static void index(int n, int i) {
        if (i < 0) throw new IndexOutOfBoundsException("index i=" + i + " < 0");
        if (n <= i) throw new IndexOutOfBoundsException("index i=" + i + " >= n=" + n);
    }
