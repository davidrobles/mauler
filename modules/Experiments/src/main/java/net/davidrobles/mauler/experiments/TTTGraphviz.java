...
/**
 * Runs UCT (1 000 simulations) from a near-terminal TicTacToe position and writes the resulting
 * search tree to {@code tree.dot} in the current working directory.
 *
 ...
 * </pre>
 *
 * X has two immediate wins (diagonal 0-4-8 via cell 8, anti-diagonal 2-4-6 via cell 6). UCT should
 * discover both and prefer one of them.
 *
...
public class TTTGraphviz {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 5; i++) game.makeMove(0); // fills cells 0-5 alternating X/O
        ...
        System.out.printf(
                "UCT chose move index %d (cell %d).%n", move, move == 0 ? 6 : move == 1 ? 7 : 8);
     ...
     *
     * <ul>
     *   <li>X — deep red on rose background; winning cells: white X on full red
     *   <li>O — deep blue on sky background; winning cells: white O on full blue
     *   <li>Empty — off-white
     *   <li>Warm gray grout shows through the cell gaps
     * </ul>
     */
    private static String tttLabel(MCTSNode<TicTacToe> node) {
        ...
        html.append(
                "<<TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLSPACING=\"3\" CELLPADDING=\"10\" BGCOLOR=\"#BDB5AC\">");
            ...
            for (int col = 0; col < TicTacToe.SIZE; col++) {
                ...
                if (cell == TicTacToe.Cell.CROSS) {
                    ...
                    html.append(String.format("<TD BGCOLOR=\"%s\">", bg))
                            .append(
                                    String.format(
                                            "<FONT FACE=\"Helvetica\" COLOR=\"%s\" POINT-SIZE=\"22\"><B>X</B></FONT>",
                                            fg))
                            .append("</TD>");
                    ...
                    html.append(String.format("<TD BGCOLOR=\"%s\">", bg))
                            .append(
                                    String.format(
                                            "<FONT FACE=\"Helvetica\" COLOR=\"%s\" POINT-SIZE=\"22\"><B>O</B></FONT>",
                                            fg))
                            .append("</TD>");
                } else {
