...
public class Connect4 implements Game<Connect4> {
    ...
    private static final long LEFT_MASK = -4432676798594L,
            RIGHT_MASK = -283691315109953L,
            UP_MASK = -128L,
            DOWN_MASK = -4363686772737L;
    ...
    public Connect4(int rows, int cols, int length) {
        ...
        if (cellsCount > 64) throw new IllegalArgumentException();
    ...
    public Cell getCell(int cellIndex) {
        ...
        if ((player1 & (1L << cellIndex)) != 0L) return Cell.P1;

        // white stone in the cell
        if ((player2 & (1L << cellIndex)) != 0L) return Cell.P2;
    ...
    public Cell[] getBoard() {
        ...
        for (int i = 0; i < cellsCount; i++) boardArray[i] = getCell(i);
    ...
    public List<Long> getBitMoves() {
        if (moves.isEmpty())
            for (int i = 0; i < cellsCount; i++) if ((legal & (1L << i)) != 0L) moves.add(1L << i);
    ...
    private void setCurrentPlayerBitboard(long bitboard) {
        if (current == 0) player1 = bitboard;
        else player2 = bitboard;
    }
    ...
    public boolean checkWin(long bitboard, long moveBitboard) {
        ...
        while (true) {
            ...
            if ((current & bitboard) == 0L) break;

            if (++howMany == n) return true;
        }
    ...
    public List<String> getMoves() {
        ...
        for (int i = 0; i < getBitMoves().size(); i++) moves.add(String.valueOf(i));
    ...
    public String toString() {
        ...
        for (long i = 0; i < cellsCount; i++) {
            if ((player1 & (1L << i)) != 0L) builder.append(" \u25C9 ");
            else if ((player2 & (1L << i)) != 0L) builder.append(" \u25CE ");
            else if ((legal & (1L << i)) != 0L) builder.append(" x ");
            else builder.append(" - ");

            if (i % cols == cols - 1) builder.append("\n");
        }
    ...
    public void printBitBoard(Long b) {
        for (long i = 0; i < cellsCount; i++) {
            if ((b & (1L << i)) != 0) System.out.print(" X ");
