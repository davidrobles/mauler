...
public class NTUtil {
    ...
    public static void randomiseWeights(double[] weights) {
        ...
        for (int i = 0; i < weights.length; i++) weights[i] = ((2 * rng.nextDouble()) - 1) / 8;
    }

    /** Returns a list of tuples by applying all the possible transformations to the given tuple. */
    public static int[][] expandTuple(int[] tuple) {
        Set<int[]> expansions = new HashSet<int[]>();
        expansions.add(tuple);

        for (NTTransform trans : NTTransform.values()) expansions.add(expand(tuple, trans));
    ...
    /**
     * Performs a random walk and returns the visited squares. The same square can be visited more
     * than once, but added to the visited squares only during the first visit.
     *
    ...
    public static int[] generateRandomWalk(int steps) {
        ...
        for (int i = 1; i < steps; i++) {
            ...
            if (!walkList.contains(cellIndex)) walkList.add(cellIndex);
        }
    ...
    /** Returns a new square bitboard by taking a random step on the given squareBitboard. */
    ...
    /**
     * Performs a random walk, creates a tuple of the visited squares, creates all the expansions of
     * that tuple and returns an NTuple with the walk and the expansions.
     */
    ...
    public static NTuple[] createNTuples(int[][] tuples) {
        ...
        for (int i = 0; i < tuples.length; i++) nTuples[i] = new NTuple(tuples[i]);

        return nTuples;
    }

    /**
     * Returns an array of {@link NTuple} from the given <code>bitTuples</code>. Ready to be used to
     * create an {@link NTupleSystem}.
     */
    public static NTuple[] createNTuples(long[] bitTuples) {
        ...
        for (int i = 0; i < bitTuples.length; i++) {
            ...
            for (int j = 0, count = 0; j < Othello.NUM_SQUARES; j++)
                if (((1L << j) & bitTuples[i]) != 0L) cells[count++] = j;
    ...
    public static NTuple[] generateNTuples(int numNTuples, int steps) {
        ...
        for (int i = 0; i < numNTuples; i++) nTuples[i] = generateNTuple(steps);
    ...
    public static int[] boardToNTupleValues(Othello othello) {
        ...
        for (int i = 0; i < cells.length; i++) {
            ...
            if (cell == Othello.Square.BLACK) board[i] = BLACK_VALUE;
            else if (cell == Othello.Square.EMPTY) board[i] = EMPTY_VALUE;
            else if (cell == Othello.Square.WHITE) board[i] = WHITE_VALUE;
        }
    ...
    private static int[][] walksSetToMultiArray(Set<int[]> walks) {
        ...
        for (int[] expansion : walks) expansionsArray[i++] = expansion;

        return expansionsArray;
    }

    /**
     * Returns an array of integers from a {@link java.util.List} of integers.
     *
    ...
    private static int[] intListToArray(List<Integer> list) {
        ...
        for (int i = 0; i < newArray.length; i++) newArray[i] = list.get(i);
    ...
    /**
     * Prints the squares on the Othello board of the given tuple. Printed to the standard output.
     */
    public static void printWalk(int[] tuple) {
        long walksBitboard = 0L;

        for (Integer cellIndex : tuple) walksBitboard |= 1L << cellIndex;
    ...
    public static void printNTupleSystem(NTupleSystem nts) {
        for (NTuple nTuple : nts.getNTuples()) {
            for (int[] tuple : nTuple.getTuples()) NTUtil.printWalk(tuple);
    ...
    public static void printWeightsPerLength() {
        ...
        for (int i = 1; i <= 10; i++) System.out.format("%10s %10s\n", i, Math.pow(3, i));
    }
