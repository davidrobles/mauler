...
public class OthelloView extends JPanel implements MoveObserver, MatchControllerObserver<Othello> {
    ...
    protected void paintComponent(Graphics gg) {
        ...
        for (int col = 0; col < Othello.SIZE; col++) {
            ...
            //            g.drawString(String.valueOf(Othello.letters[col]), BOARD_X_OFFSET + col *
            // CELL_SIZE + x,
        ...
        for (int row = 0; row < Othello.SIZE; row++) {
            ...
            //            g.drawString(str, panelWidth - BOARD_X_OFFSET + x, BOARD_Y_OFFSET + row *
            // CELL_SIZE + y);
        }
        ...
        String pieceCountStr =
                "Black: " + othello.getNumDiscs(0) + " White: " + othello.getNumDiscs(1);
        ...
        if (othello.isOver()) {
            ...
            if (othello.getOutcome().orElseThrow()[0] == GameResult.WIN) msg = "BLACK wins";
            else if (othello.getOutcome().orElseThrow()[1] == GameResult.WIN) msg = "WHITE wins";
            else msg = "Draw!";
    ...
    private int getCol(char letter) {
        for (int i = 0; i < Othello.SIZE; i++) if ((char) ('A' + (i)) == letter) return i;
