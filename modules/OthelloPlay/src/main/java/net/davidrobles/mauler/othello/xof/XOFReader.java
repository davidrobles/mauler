...
/**
 * Utility methods to read XOF files. XOF is the XML Othello game file format used into OthBase as
 * the primary supported file format for mauler databases.
 */
public class XOFReader {
    static String[] loadGames(String filename) throws FileNotFoundException {
        ...
        while (scanner.hasNextLine()) games.add(scanner.nextLine());
    ...
    static boolean checkGame(Othello othello, String[] moves, boolean debug) {
        ...
        while (!othello.isOver()) {
            ...
            if (!checkMove(othello, myMove)) return false;

            othello.makeMove(othello.getMoves().indexOf(myMove));

            if (debug) System.out.println(othello);
    ...
    static String[] breakMoves(String movesStr) {
        ...
        for (int i = 0; i < moves.length; i++)
            moves[i] =
                    String.valueOf(movesStr.charAt(i * 2))
                            + String.valueOf(movesStr.charAt(i * 2 + 1));
    ...
    static void checkGames(String[] games, boolean debug) {
        ...
        for (String gameStr : games) {
            ...
            if (!checkGame(othello, breakMoves(gameStr), debug)) System.out.println(gameStr);
        }
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        checkGame(
                new Othello(),
                breakMoves(
                        "f5d6c3d3c4f4f6g5e6f7d7c5g3e3d2c7b5b6c6d8e7f3a6h3g4e8h6b4a5a3a4a7g6c2b2b3f2h5a2a1e2h4h2b1c1f1e1b7h7d1g2h1g1h8b8g7a8c8f8g8"),
                true);
    }
}
