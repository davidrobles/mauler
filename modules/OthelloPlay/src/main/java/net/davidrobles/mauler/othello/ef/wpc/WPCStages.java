...
public class WPCStages implements Evaluator<Othello> {
    ...
    public WPCStages(List<? extends WPC> wpcs, int[] ranges, Random rng) {
        if (wpcs.size() - 1 != ranges.length) throw new IllegalArgumentException();
    ...
    public double evaluate(Othello othello, int player) {
        ...
        if (wpc == null) return rng.nextDouble() * 2 - 1;

        return wpc.evaluate(othello, player);
    }

    public static void validateNumStones(int stones) {
        if (stones < 4 || stones > 64) throw new IllegalArgumentException();
    }
    ...
    public static void validateRanges(int[] ranges) {
        ...
        for (int i = 0; i < ranges.length; i++) {
            if (ranges[i] <= 0 || ranges[i] > 60) throw new IllegalArgumentException();

            if (i > 0 && ranges[i - 1] >= ranges[i]) throw new IllegalArgumentException();
        }
    }
    ...
    public static int findStage(int stones, int[] ranges) {
        ...
        for (int i = 0; i < ranges.length + 1; i++) {
            if (i == 0) {
                if (stones >= Othello.NUM_DISCS_START
                        && stones < ranges[i] + Othello.NUM_DISCS_START) return i;
            } else if (i == ranges.length) {
                if (stones >= ranges[i - 1] + Othello.NUM_DISCS_START
                        && stones <= Othello.NUM_SQUARES) return i;
            } else {
                if (stones >= ranges[i - 1] + Othello.NUM_DISCS_START
                        && stones < ranges[i] + Othello.NUM_DISCS_START) return i;
            }
