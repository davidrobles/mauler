...
public class NTuple implements LinearEF<Othello> {
    ...
    /**
     * Creates an N-tuple with the given sample points. It automatically creates all the expansions
     * for the given tuple.
     */
    ...
    /**
     * Creates an N-tuple with the given tuples. The tuples are usually the expansions of one of the
     * tuples, but it is not a requirement.
     */
    ...
    public NTuple copy() {
        ...
        for (int i = 0; i < tuplesCopy.length; i++)
            for (int j = 0; j < tuplesCopy[i].length; j++) tuplesCopy[i][j] = tuples[i][j];
    ...
    public String toBoardStr() {
        ...
        for (int row = 0; row < tuples.length; row++)
            for (int col = 0; col < tuples[row].length; col++) discs.add(tuples[row][col]);
        ...
        for (int disc = 0; disc < Othello.NUM_SQUARES; disc++) {
            if (list.contains(disc)) builder.append("p");
            else builder.append("-");

            if (disc % Othello.SIZE == 7) builder.append("\n");
        }
    ...
    public void updateWeights(Othello othello, double tdError) {
        ...
        for (int[] tuple : tuples) weights[address(tuple, board)] += tdError;
    }
    ...
    public double evaluate(Othello othello, int player) {
        ...
        for (int[] tuple : tuples) value += weights[address(tuple, board)];
