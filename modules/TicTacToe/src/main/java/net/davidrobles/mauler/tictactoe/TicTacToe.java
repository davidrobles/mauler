 ...
 *
 * <p>Bit {@code i} in {@code crosses} (resp. {@code noughts}) is set when player 0 (resp. player 1)
 * occupies cell {@code i}, where cells are numbered row-major:
 *
 * <pre>
 *   0 | 1 | 2
 *  ---+---+---
...
public class TicTacToe extends ObservableGame<TicTacToe> {
    ...
    public Cell[] getBoard() {
        ...
        for (int i = 0; i < CELLS; i++) board[i] = getCell(i);
        return board;
    }
    ...
    private boolean checkWin(int board) {
        for (int pattern : WINNING_PATTERNS) if ((board & pattern) == pattern) return true;
        return false;
    }
    ...
    private void setCurrentBitboard(int bitboard) {
        if (getCurPlayer() == 0) crosses = bitboard;
        else noughts = bitboard;
    }
    ...
    private List<Integer> legalMoveIndices() {
        ...
        if (getNumMoves() > 0) {
            ...
            for (int i = 0; i < CELLS; i++) if ((empty & (1 << i)) != 0) moves.add(i);
        }
        return moves;
    }
    ...
    public List<String> getMoves() {
        ...
        for (int i = 0; i < indices.size(); i++) moves.add(String.valueOf(indices.get(i)));
    ...
    public String toString() {
        ...
        if (!isOver()) {
            ...
            sb.append("Moves:  ")
                    .append(Arrays.toString(legalMoveIndices().toArray()))
                    .append("\n");
        } else {
            sb.append("Game over\n");
        }
        ...
        for (int i = 0; i < CELLS; i++) {
            if ((crosses & (1 << i)) != 0) sb.append(" X ");
            else if ((noughts & (1 << i)) != 0) sb.append(" O ");
            else sb.append(" - ");
