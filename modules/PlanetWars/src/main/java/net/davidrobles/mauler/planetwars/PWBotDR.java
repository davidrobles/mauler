...
public class PWBotDR implements PWBot {
    ...
    // Fleet comparators
    //    private static final Comparator<PWFleet> FLEETS_SORT_BY_NUM_SHIPS = new
    // PWFleetSortByNumShips();
    //    private static final Comparator<PWFleet> FLEETS_SORT_BY_TURNS_REMAINING = new
    // PWFleetSortByTurnsRemaining();
    ...
    private List<PWOrder> expand2() {
        ...
        for (PWPlanet myPlanet : gameState.myPlanets(playerID)) {
            ...
            // myPlanet.getPlanetID(), neighbor.getPlanetID(),
    ...
    public boolean myFleeGoingToPlanet(PWPlanet planet) {
        ...
        for (PWFleet fleet : myFleets) {
            if (fleet.getOwner() != playerID
                    || (fleet.getOwner() == playerID
                            && fleet.getDestinationPlanetID() == planet.getPlanetID())) {
                return true;
            }
        }
        return false;
    ...
    private List<PWOrder> expand() {
        ...
        LinkedList<PWPlanet> myPlanetsByNumShipsDesc =
                myPlanetsSorted(PLANETS_SORT_BY_NUM_SHIPS_DESC);
        ...
        // Expand
        LinkedList<PWPlanet> neutralPlanetsByNumShipsAsc =
                neutralPlanetsSorted(PLANETS_SORT_BY_NUM_SHIPS_ASC);
        LinkedList<PWPlanet> neutralPlanetsByDistAsc =
                neutralPlanetsSorted(PLANETS_SORT_BY_DIST_ASC);
        List<PWPlanet> planetsToExpand = new ArrayList<PWPlanet>();
        int shipsToSend = 0;

        for (PWPlanet planet : neutralPlanetsByDistAsc) {
            //            System.out.println("Planet dist: "+
    ...
    private List<PWOrder> attack() {
        ...
        LinkedList<PWPlanet> oppPlanetsByNumShipsAsc =
                oppPlanetsSorted(PLANETS_SORT_BY_NUM_SHIPS_ASC);
        ...
        for (Map.Entry<PWPlanet, Integer> entry : map.entrySet()) {
            orders.add(
                    new PWOrder(
                            playerID,
                            entry.getKey().getPlanetID(),
                            oppWeakestPlanet.getPlanetID(),
                            entry.getValue()));
        }
