...
public class PWView extends JPanel implements PWGameObserver {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (PWPlanet planet : game.getGameState().getAllPlanets()) {
            // Draw planets
            if (planet.getOwner() == 1) g.setColor(P1_COLOR);
            else if (planet.getOwner() == 2) g.setColor(P2_COLOR);
            else g.setColor(NEUTRAL_COLOR);
            ...
            g.drawString(
                    String.valueOf(planet.getNumShips()) + "(" + planet.getGrowthRate() + ")",
                    xPos + LEFT_OFFSET + (planetSize / 2),
                    yPos + TOP_OFFSET + (planetSize / 2));
        }

        for (PWFleet fleet : game.getGameState().getAllFleets()) {
            if (fleet.getOwner() == 1) g.setColor(P1_COLOR);
            else if (fleet.getOwner() == 2) g.setColor(P2_COLOR);

            PWPlanet sourcePlanet = game.getGameState().getPlanet(fleet.getSourcePlanetID());
            PWPlanet destinationPlanet =
                    game.getGameState().getPlanet(fleet.getDestinationPlanetID());
            double xStep =
                    (destinationPlanet.getX() - sourcePlanet.getX()) / fleet.getTotalTripLength();
            double yStep =
                    (destinationPlanet.getY() - sourcePlanet.getY()) / fleet.getTotalTripLength();
            int xPos =
                    (int)
                            ((destinationPlanet.getX() - (xStep * fleet.getTurnsRemaining()))
                                    * SCALE);
            int yPos =
                    (int)
                            ((destinationPlanet.getY() - (yStep * fleet.getTurnsRemaining()))
                                    * SCALE);
            g.drawString(
                    String.valueOf(fleet.getNumShips()), xPos + LEFT_OFFSET, yPos + TOP_OFFSET);
        }
    }
