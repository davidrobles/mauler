...
public class PWGameState {
    ...
    public PWGameState(PWGameState gameState) {
        // Copy planets
        for (PWPlanet planet : gameState.getAllPlanets()) planets.add(new PWPlanet(planet));
        // Copy fleets
        for (PWFleet fleet : gameState.getAllFleets()) fleets.add(new PWFleet(fleet));
        // Copy rest stuff
    ...
    public List<PWPlanet> getAllPlanets() {
        ...
        for (PWPlanet p : planets) r.add(p);
        return r;
    }
    ...
    public LinkedList<PWPlanet> growth5Planets() {
        LinkedList<PWPlanet> r = new LinkedList<PWPlanet>();
        for (PWPlanet p : planets) if (p.getGrowthRate() == 5) r.add(p);
        return r;
    }

    public LinkedList<PWPlanet> myPlanets(int playerIndex) {
        LinkedList<PWPlanet> r = new LinkedList<PWPlanet>();
        for (PWPlanet p : planets) if (p.getOwner() == playerIndex) r.add(p);
        return r;
    }

    public List<PWPlanet> notMyPlanets(int playerIndex) {
        List<PWPlanet> notMyPlanets = new ArrayList<PWPlanet>();
        for (PWPlanet p : planets) if (p.getOwner() != playerIndex) notMyPlanets.add(p);
        return notMyPlanets;
    }

    public LinkedList<PWPlanet> oppPlanets(int playerIndex) {
        LinkedList<PWPlanet> r = new LinkedList<PWPlanet>();
        for (PWPlanet p : planets) if (p.getOwner() != playerIndex && p.getOwner() != 0) r.add(p);
        return r;
    }

    public LinkedList<PWPlanet> neutralPlanets() {
        LinkedList<PWPlanet> r = new LinkedList<PWPlanet>();
        for (PWPlanet p : planets) if (p.getOwner() == 0) r.add(p);
        return r;
    }
    ...
    private int parseGameState(String s) {
        ...
        for (String line : lines) {
            ...
            if (commentBegin >= 0) line = line.substring(0, commentBegin);
    ...
    public int NumShips(int playerID) {
        ...
        for (PWPlanet p : planets) if (p.getOwner() == playerID) numShips += p.getNumShips();

        for (PWFleet f : fleets) if (f.getOwner() == playerID) numShips += f.getNumShips();
    ...
    public int Winner() {
        ...
        for (PWPlanet p : planets) remainingPlayers.add(p.getOwner());

        for (PWFleet f : fleets) remainingPlayers.add(f.getOwner());
    ...
    public void doTimeStep() {
        ...
        for (PWPlanet p : planets) if (p.getOwner() > 0) p.addShips(p.getGrowthRate());

        // Advance all fleets by one time step.
        for (PWFleet f : fleets) f.move();

        // Determine the result of any battles
        for (PWPlanet p : planets) FightBattle(p);
        ...
        while (it.hasNext()) {
            ...
            if (f.getTurnsRemaining() <= 0 && getPlanet(f.getDestinationPlanetID()) == p) {
                if (!participants.containsKey(f.getOwner())) {
                    ...
                    participants.put(
                            f.getOwner(), f.getNumShips() + participants.get(f.getOwner()));
                }
                it.remove();
            }
        }
    ...
    public void makeMoves(List<PWOrder> orders) {
        for (PWOrder order : orders) {
            IssueOrder(
                    order.getPlayerID(),
                    order.getSourcePlanet(),
                    order.getDestinationPlanet(),
                    order.getNumShips());
        }
    }
