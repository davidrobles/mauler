...
public class PWBotDef implements PWBot {
    ...
    private static final PWPlanetSortByDistance PLANETS_SORT_BY_DIST_ASC =
            new PWPlanetSortByDistance();

    private static final PWPlanetSortByDistanceAll PLANETS_SORT_BY_DIST_ALL_ASC =
            new PWPlanetSortByDistanceAll();
    ...
    private Map<PWPlanet, Integer> fightPlanetFromMyAllPlanets(PWPlanet targetPlanet) {
        ...
        for (PWPlanet myPlanet : myPlanets) shipsFrom += myPlanet.getNumShips();
        ...
        for (PWPlanet myPlanet : myPlanets) {
            ...
            //                    + (gameState.distance(myPlanet.getPlanetID(),
            // targetPlanet.getPlanetID()) * targetPlanet.getGrowthRate());
    ...
    private int shipsComing(PWPlanet planet) {
        ...
        for (PWFleet fleet : gameState.getAllFleets()) {
            if (fleet.getDestinationPlanetID() == planet.getPlanetID()) {
                if (fleet.getOwner() == 1) balance += fleet.getNumShips();
                else balance -= fleet.getNumShips();
            }
        }
        return balance;
    }
    ...
    public List<PWOrder> getOrders(PWGameState gameState) {
        ...
        for (PWPlanet myPlanet : myPlanets) numMyShipsInPlanets += myPlanet.getNumShips();
        ...
        for (PWPlanet targetCandidate : notMyPlanets) {
            if (turns < 30 && targetCandidate.getNumShips() > 40) continue;
        ...
        for (PWPlanet targetPlanet : targetPlanets) {
            ...
            for (PWPlanet nearPlanet : nearestMyPlanets) {
                if (myPlanetsMap.containsKey(nearPlanet.getPlanetID())
                        && myPlanetsMap.get(nearPlanet.getPlanetID()) > 1) {
                    ...
                    int extraGrowthShips =
                            (gameState.distance(
                                                    nearPlanet.getPlanetID(),
                                                    targetPlanet.getPlanetID())
                                            * targetPlanet.getGrowthRate())
                                    + 1;
                    if (targetPlanet.getOwner() == 2) shipsToSendInc += extraGrowthShips;
        ...
        //        }
        //
        //        Map<PWPlanet, Integer> myAttackingPlanets =
        // fightPlanetFromMyAllPlanets(nearestPlanet);
        //
        //        for (Map.Entry<PWPlanet, Integer> entry : myAttackingPlanets.entrySet()) {
        //            orders.add(new PWOrder(playerID, entry.getKey().getPlanetID(),
        // nearestPlanet.getPlanetID(), entry.getValue()));
        //        }
        ...
        //                if (gameState.distance(firstMyPlanet.getPlanetID(), planet.getPlanetID())
        // < 10
        ...
        //                        && gameState.distance(strongestPlanet.getPlanetID(),
        // dstPlanet.getPlanetID()) < 15) {
        //                    int shipsToNeighbor = 0;
        //
        //                    if (dstPlanet.getNumShips() < (strongestPlanet.getNumShips() -
        // shipsAllocated - 2)) {
        //                        if (dstPlanet.getOwner() == 2) {
        //                            // TODO: bug when sending more ships than allowed
        //                            shipsToNeighbor = dstPlanet.getNumShips()
        //                                    + (gameState.distance(strongestPlanet.getPlanetID(),
        //                                    dstPlanet.getPlanetID()) * dstPlanet.getGrowthRate())
        // + 1;
        //                        } else {
        //                            shipsToNeighbor = dstPlanet.getNumShips() + 1;
        //                        }
        //                        orders.add(new PWOrder(playerID, strongestPlanet.getPlanetID(),
        // dstPlanet.getPlanetID(),
        //                                shipsToNeighbor));
        //                        shipsAllocated += shipsToNeighbor;
        //                    } else if ((strongestPlanet.getNumShips() - shipsAllocated - 2) >= 10)
        // {
        //                        shipsToNeighbor = (strongestPlanet.getNumShips() - shipsAllocated)
        // - 1;
        //                        orders.add(new PWOrder(playerID, strongestPlanet.getPlanetID(),
        // dstPlanet.getPlanetID(),
        //                                shipsToNeighbor));
        ...
        // shipsAllocated - 2)) {
        //                            shipsToNeighbor = dstPlanet.getNumShips() + 1;
        //                            orders.add(new PWOrder(playerID,
        // strongestPlanet.getPlanetID(), dstPlanet.getPlanetID(),
        //                                    shipsToNeighbor));
        //                            shipsAllocated += shipsToNeighbor;
        //                        } else if ((strongestPlanet.getNumShips() - shipsAllocated - 2) >=
        // 10) {
        //                            shipsToNeighbor = (strongestPlanet.getNumShips() -
        // shipsAllocated) - 1;
        //                            orders.add(new PWOrder(playerID,
        // strongestPlanet.getPlanetID(), dstPlanet.getPlanetID(),
        //                                    shipsToNeighbor));
        ...
        //                    orders.add(new PWOrder(playerID, myPlanet.getPlanetID(),
        // bestPlanet.getPlanetID(),
        ...
        //                        if (gameState.distance(myPlanet.getPlanetID(),
        // otherPlanet.getPlanetID()) < bestScore
        //                                && otherPlanet.getOwner() != playerID) {
        //                            nearestPlanet = otherPlanet;
        //                            bestScore = gameState.distance(myPlanet.getPlanetID(),
        // otherPlanet.getPlanetID());
        //                        }
        //                    }
        //                }
        //            }
        ...
        //                for (PWPlanet dstPlanet : allPlanets)
        //                {
        //                    System.out.println(gameState.distance(srcPlanet.getPlanetID(),
        // dstPlanet.getPlanetID()));
        //                    if (dstPlanet.getOwner() == 0 && dstPlanet != srcPlanet
        //                            && gameState.distance(srcPlanet.getPlanetID(),
        // dstPlanet.getPlanetID()) < 4)
        //                    {
        //                        int shipsToNeighbor = 0;
        //
        //                        if (dstPlanet.getNumShips() < (srcPlanet.getNumShips() -
        // shipsAllocated - 2)) {
        //                            shipsToNeighbor = dstPlanet.getNumShips() + 1;
        //                            orders.add(new PWOrder(playerID, srcPlanet.getPlanetID(),
        // dstPlanet.getPlanetID(),
        //                                    shipsToNeighbor));
        //                            shipsAllocated += shipsToNeighbor;
        //                        } else if ((srcPlanet.getNumShips() - shipsAllocated - 2) >= 10) {
        //                            shipsToNeighbor = (srcPlanet.getNumShips() - shipsAllocated) -
        // 1;
        //                            orders.add(new PWOrder(playerID, srcPlanet.getPlanetID(),
        // dstPlanet.getPlanetID(),
        //                                    shipsToNeighbor));
