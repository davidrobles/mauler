...
public class PWBotMC implements PWBot {
    ...
    public List<PWOrder> getOrders(PWGameState gameState) {
        ...
        for (PWMove move : myMoves) {
            ...
            for (int i = 0; i < PLAYS_PER_MOVE; i++) {
                ...
                if (move.getSrcPlanet() != null) {
                    playOrders.add(
                            new PWOrder(
                                    move.getSrcPlanet().getOwner(),
                                    move.getSrcPlanet().getPlanetID(),
                                    move.getDstPlanet().getPlanetID(),
                                    move.getnShips()));
                }
                ...
                if (oppMove.getSrcPlanet() != null) {
                    playOrders.add(
                            new PWOrder(
                                    oppMove.getSrcPlanet().getOwner(),
                                    oppMove.getSrcPlanet().getPlanetID(),
                                    oppMove.getDstPlanet().getPlanetID(),
                                    oppMove.getnShips()));
                }
                // Make first moves
                ...
                while (playGameState.Winner() < 0
                        && playGameState.getTurns() < (turnsBefore + 100)) {
                    playOrders.clear();
                    // Add my move
                    ...
                    if (myMove.getSrcPlanet() != null) {
                        playOrders.add(
                                new PWOrder(
                                        myMove.getSrcPlanet().getOwner(),
                                        myMove.getSrcPlanet().getPlanetID(),
                                        myMove.getDstPlanet().getPlanetID(),
                                        myMove.getnShips()));
                    }
                    // Add opp move
                    ...
                    if (oppFirstMove.getSrcPlanet() != null) {
                        playOrders.add(
                                new PWOrder(
                                        oppFirstMove.getSrcPlanet().getOwner(),
                                        oppFirstMove.getSrcPlanet().getPlanetID(),
                                        oppFirstMove.getDstPlanet().getPlanetID(),
        ...
        if (bestMove.getSrcPlanet() != null) {
            orders.add(
                    new PWOrder(
                            playerID,
                            bestMove.getSrcPlanet().getPlanetID(),
                            bestMove.getDstPlanet().getPlanetID(),
                            bestMove.getnShips()));
        }
    ...
    double evaluate(PWGameState gameState, int playerIndex) {
        if (gameState.NumShips(1) > gameState.NumShips(2)) return 1.0;
        else return -1.0;
    }
