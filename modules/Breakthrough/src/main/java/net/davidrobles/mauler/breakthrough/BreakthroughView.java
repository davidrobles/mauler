...
public class BreakthroughView extends JPanel
        implements MoveObserver, MatchControllerObserver<Breakthrough>, MouseListener {
    ...
    protected void paintComponent(Graphics gg) {
        ...
        //                            row * CELL_SIZE + BOARD_Y_OFFSET + CELL_Y_OFFSET, CELL_SIZE -
        // (CELL_X_OFFSET * 2),
    ...
    private void drawStatus(Graphics2D g) {
        ...
        g.drawString(
                playerTurnStr,
                panelWidth - (int) playerTurnRect.getWidth(),
                panelHeight - (int) pieceCountStrRect.getHeight() / 2);
    }
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int col = 0; col < bk.getCols(); col++)
            for (int row = 0; row < bk.getRows(); row++)
                g.drawRect(
                        col * CELL_SIZE + BOARD_X_OFFSET,
                        row * CELL_SIZE + BOARD_Y_OFFSET,
                        CELL_SIZE,
                        CELL_SIZE);
    }
    ...
    private void drawLegalMoves(Graphics2D g) {
        if (row != -1 && col != -1) {
            ...
            for (String moveStr : mv) {
                ...
                g.fillOval(
                        pointCol * CELL_SIZE + BOARD_X_OFFSET + CELL_X_OFFSET,
                        pointRow * CELL_SIZE + BOARD_Y_OFFSET + CELL_Y_OFFSET,
    ...
    private void drawLetters(Graphics2D g) {
        ...
        for (int col = 0; col < bk.getCols(); col++) {
            ...
            g.drawString(
                    String.valueOf((char) ('a' + (col))), BOARD_X_OFFSET + col * CELL_SIZE + x, y);
        }
        ...
        for (int row = 0; row < bk.getRows(); row++) {
            for (int col = 0; col < bk.getCols(); col++) {
                if (bk.getCell(row, col) != Cell.EMPTY) {
                    if (bk.getCell(row, col) == Cell.BLACK) g.setColor(Color.BLACK);
                    else if (bk.getCell(row, col) == Cell.WHITE) g.setColor(Color.WHITE);

                    g.fillOval(
                            col * CELL_SIZE + BOARD_X_OFFSET + CELL_X_OFFSET,
    ...
    private List<String> filterMoves(int row, int col) {
        ...
        for (String move : bk.getMoves())
            if (bk.cellToString(row, col).equals(move.substring(0, 2))) newMoves.add(move);
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (coordValid(row, col)) System.out.println("index " + bk.cellToString(row, col));
