...
/**
 * A player that acts greedily with probability {@code (1 - ε)} and uniformly at random with
 * probability {@code ε}.
 *
 * <p>Special cases:
 *
 * <ul>
 *   <li>{@code ε = 0.0} — fully greedy (deterministic); equivalent to {@link GreedyStrategy}
 *   <li>{@code ε = 1.0} — fully random; equivalent to {@link RandomStrategy}
 * </ul>
 *
 * <p>Commonly used as a rollout policy inside MCTS to inject heuristic knowledge while keeping some
 * exploration. The seeded-{@link Random} constructor enables reproducible experiments.
 *
