...
public final class Games {
    ...
    /**
     * Advances {@code game} by up to {@code numMoves} random moves, stopping early if the game
     * ends.
     *
     * @return {@code true} if all {@code numMoves} moves were made, {@code false} if the game ended
     *     before reaching that count
     */
    ...
    /**
     * Plays a complete game between the given strategies, printing every move and the final outcome
     * to stdout.
     *
     * <p>The game is reset before play begins. Each strategy receives a defensive copy of the
     * current state so it cannot mutate the live game.
     *
    ...
    /**
     * Plays a complete game with uniformly random moves for all players, printing every move and
     * the final outcome to stdout.
     *
    ...
    private static String outcomeLabel(GameResult outcome) {
        switch (outcome) {
            case WIN:
                return fmt("WIN", BOLD, BRIGHT_GREEN);
            case LOSS:
                return fmt("LOSS", BOLD, BRIGHT_RED);
            case DRAW:
                return fmt("DRAW", BOLD, BRIGHT_YELLOW);
            default:
                return dim("N/A");
        }
    }
