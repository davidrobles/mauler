...
public final class StrategiesUtil {
    ...
    /**
     * Selects a move using the ε-greedy policy: greedy with probability {@code (1 - epsilon)},
     * uniformly random with probability {@code epsilon}.
     *
    ...
    public static <GAME extends Game<GAME>> int epsilonGreedyMove(
        ...
        if (rng.nextDouble() > epsilon) return greedyMove(game, evalFunc);
    ...
    /**
     * Selects the move leading to the highest-scored successor state. Ties are broken in favor of
     * the first move encountered.
     *
     * @param game the current game state
    ...
    /**
     * Returns a numeric score for {@code player} in a finished game using caller-supplied
        ...
        switch (game.getOutcome().orElseThrow()[player]) {
            ...
            default:
                throw new IllegalStateException(
                        "Unknown GameResult: " + game.getOutcome().orElseThrow()[player]);
        }
    }
}
