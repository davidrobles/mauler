...
public class BoardApp<GAME extends Game<GAME> & MoveObservable> extends JFrame {
    ...
    public BoardApp(
            JPanel gameView,
            final MatchController<GAME> controller,
            MatchControllerButtonsView<GAME> buttonsView,
            MatchControllerSliderView<GAME> sliderView,
            JPanel movesView) {
        // Menu bar
        ...
        fileMenuNewItem.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        if (e.getSource() == fileMenuNewItem) controller.reset();
                    }
                });
        fileMenu.add(fileMenuNewItem);
        final JMenuItem fileMenuSaveGameItem = new JMenuItem("Save...");
        fileMenuSaveGameItem.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        if (e.getSource() == fileMenuSaveGameItem) controller.reset();
                    }
                });
