package net.davidrobles.mauler.strategies.mc;
 ...
 *
 * <p>For each legal move, a number of random rollouts are run to completion and scored with a
 * {@link TerminalEvaluator}. The move with the highest cumulative score is returned.
 *
 * <p>Two operational modes are supported:
 *
 * <ul>
 *   <li><b>Simulation-count</b> — construct with {@code nSims}; distributes simulations round-robin
 *       across all moves, then picks the best.
 *   <li><b>Time-based</b> — construct without {@code nSims}; use {@link #move(Game, int)} with a
 *       millisecond budget. Runs full sweeps over all moves until time expires.
 * </ul>
 *
...
public class MonteCarlo<GAME extends Game<GAME>> implements Strategy<GAME> {
    ...
    /**
     * Creates a time-based player backed by a default (unseeded) {@link Random}. Use {@link
     * #move(Game, int)} — calling {@link #move(Game)} will throw.
     */
    public MonteCarlo() {
        this(new Random());
    }
    ...
    /**
     * Creates a time-based player backed by the given {@link Random}. Use {@link #move(Game, int)}
     * — calling {@link #move(Game)} will throw.
     *
    ...
    /**
     * Selects a move using the configured number of rollouts, distributed round-robin across all
     * legal moves.
     *
    ...
    /**
     * Selects a move by running rollouts until the time budget is exhausted. Each sweep simulates
     * every legal move once before repeating.
     */
    @Override
    ...
    /**
     * Plays {@code move} on a copy of {@code game}, then runs a random rollout to completion and
     * returns the score for {@code player}.
     */
    private double simulate(GAME game, int move, int player) {
        GAME copy = game.copy();
        copy.makeMove(move);

        while (!copy.isOver()) copy.makeMove(rand.nextInt(copy.getNumMoves()));
    ...
    public String toString() {
        return nSims < 0 ? "<MonteCarlo>" : String.format("<MonteCarlo nSims=%d>", nSims);
    }
}
