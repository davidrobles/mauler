 ...
 *
 * <p>Implements {@link Callable} so matches can be submitted to an {@link
 * java.util.concurrent.ExecutorService} for parallel tournament execution.
 *
 * <p>The {@code starter} parameter controls which strategy moves first: {@code 0} means {@code
 * strategies.get(0)} starts, {@code 1} means {@code strategies.get(1)} starts. Outcomes are always
 * returned from {@code strategies.get(0)}'s perspective regardless of who started.
 *
 * <p>Each strategy receives a defensive copy of the game state so it cannot mutate the live game.
 *
...
public class Match<GAME extends Game<GAME>> implements Callable<GameResult[]> {
     ...
     */
    public Match(
            Supplier<GAME> gameFactory, List<Strategy<GAME>> players, int starter, int timeout) {
        if (timeout <= 0)
    ...
    public GameResult[] call() {
        ...
        while (!g.isOver()) g.makeMove(selectMove(g));

        GameResult[] outcomes = g.getOutcome().orElseThrow();

        if (starter == 1) outcomes = new GameResult[] {outcomes[1], outcomes[0]};
