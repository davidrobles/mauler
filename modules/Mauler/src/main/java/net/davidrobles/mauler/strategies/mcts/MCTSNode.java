 ...
 *
 * <p>Each node stores a game state and accumulates statistics across all simulations that pass
 * through it: visit count N(s) and mean action-value Q(s,a) from the perspective of the current
 * player at that state.
 *
 * <p>Children are created lazily via {@link #expand()}, which expands one level of successors for
...
public class MCTSNode<GAME extends Game<GAME>> {
     ...
     *
     * <p>Called externally rather than from the constructor so that MCTS can control when expansion
     * happens — expanding only the node chosen by the tree policy, not every node at construction
     * time.
     */
    public void expand() {
     ...
     *
     * @param outcome the result of the simulation, from the perspective of the node's current
     *     player (typically in [0, 1])
     */
    ...
    /**
     * Overrides the visit count. Used by prior-knowledge initialization ({@code MCTSWithPrior}) and
     * parallel root parallelization ({@code MCTSRootParallel}).
     */
    ...
    /**
     * Overrides the stored value. Used by prior-knowledge initialization ({@code MCTSWithPrior}).
     */
