 ...
 *
 * <p>Only nodes that were visited at least once are included. Each node is labeled with its visit
 * count and mean value. The output can be rendered with any Graphviz layout engine, e.g.:
 *
 * <pre>
 *   dot -Tpdf tree.dot -o tree.pdf
 * </pre>
 *
 * <p><b>Transposition handling:</b> multiple MCTS tree nodes that represent the same game state
 * (equal under {@link Object#equals}) are collapsed into a single DOT node. Statistics shown are
 * from the first-encountered tree node for that state. Games that do not override {@code equals()}
 * will fall back to identity comparison and no deduplication will occur.
 *
...
public class GraphvizMCTSObserver<GAME extends Game<GAME>> implements MCTSObserver<GAME> {
     ...
     *
     * <p>The function receives the {@link MCTSNode} and must return either a plain string (output
     * as {@code "..."}) or an HTML label (starts with {@code <}, output as {@code <...>}).
     */
    ...
    public void searchFinished(MCTSNode<GAME> root) {
        ...
        if (rootLabel.startsWith("<")) sb.append("    node [shape=none margin=0];\n");
        else sb.append("    node [shape=box fontname=Helvetica];\n");
    ...
    private void appendNodeDef(StringBuilder sb, int id, MCTSNode<GAME> node) {
        ...
        if (label.startsWith("<")) sb.append(String.format("    %d [label=%s];\n", id, label));
        else sb.append(String.format("    %d [label=\"%s\"];\n", id, label));
    }

    /**
     * Recursively writes edges (and node definitions on first encounter) for all visited children
     * of {@code node}. Each unique game state gets exactly one DOT node regardless of how many MCTS
     * tree nodes share that state.
     */
    private void writeChildren(
            MCTSNode<GAME> node, int nodeId, StringBuilder sb, Map<GAME, Integer> seen) {
        for (MCTSNode<GAME> child : node.getChildren()) {
            ...
            if (firstVisit) writeChildren(child, childId, sb, seen);
        }
    }
}
