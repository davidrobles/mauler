 ...
 *
 * <p>Spawns {@code nThreads} independent MCTS workers, each starting from the
 * same root state and building a private search tree — no shared state, no
 * synchronization. After all workers finish, their per-move statistics are
 * aggregated: visit counts are summed and values are combined as a
 * weighted mean. The final move is selected by the robust-child criterion
 * (most-visited child).
 *
 * <p>Root parallelization is the simplest of the three main parallel MCTS
 * strategies (leaf, root, tree). Its strength is zero synchronization
 * overhead; its weakness is that threads cannot share discoveries mid-search.
 *
 * <p>References:
 * <ul>
 *   <li>Chaslot et al. (2008). "Parallel Monte-Carlo Tree Search."
 *       <em>Computers and Games (CG 2008)</em>, LNCS 5131:60&ndash;71.
 *       Introduces and evaluates leaf, root, and tree parallelization.
 *   <li>Browne et al. (2012). "A Survey of Monte Carlo Tree Search Methods."
 *       <em>IEEE TCIAIG</em>, 4(1):1&ndash;43. §8 covers parallel MCTS and
...
public class MCTSRootParallel<GAME extends Game<GAME>> implements Strategy<GAME>
{
    ...
    /**
     * Merges independently built root nodes by summing child visit counts and
     * computing a weighted mean of child values:
     * <pre>
    ...
    /**
     * Returns the most-visited child index (robust-child criterion).
     * Robust child is less susceptible to statistical noise than selecting
     * the highest-value child.
     */
    ...
    /**
     * A single parallel MCTS worker. Each task receives an independent copy
     * of the MCTS configuration and builds a private search tree from scratch.
     */
    ...
    private int runParallel(List<MCTSTask> tasks)
    {
        ...
        try
        {
            for (Future<MCTSNode<GAME>> future : executor.invokeAll(tasks))
                roots.add(future.get());
        }
    ...
    /**
     * Runs {@code mcts.nSims} simulations on each of the {@code nThreads} workers
     * and returns the most-visited move after merging.
     *
     * <p>Total simulations = {@code nSims * nThreads}. Each thread receives the full
     * simulation budget independently, exploiting all available cores.
     *
     * @throws IllegalStateException if the base MCTS was configured for time-based
     *                               operation (nSims == 0); use {@link #move(Game, int)} instead
     */
    @Override
    public int move(GAME game)
    {
        if (mcts.nSims <= 0)
            throw new IllegalStateException("move(game) requires a simulation-count MCTS; use move(game, timeout) instead");

        List<MCTSTask> tasks = new ArrayList<>();

        for (int i = 0; i < nThreads; i++)
            tasks.add(new MCTSTask(game.copy(), mcts.nSims));

        return runParallel(tasks);
    }

    /**
     * Runs each of the {@code nThreads} workers for {@code timeout} milliseconds
     * simultaneously, then merges results and returns the most-visited move.
     */
    @Override
    public int move(GAME game, int timeout)
    {
        ...
        for (int i = 0; i < nThreads; i++)
            tasks.add(new MCTSTask(game.copy(), timeDue));
