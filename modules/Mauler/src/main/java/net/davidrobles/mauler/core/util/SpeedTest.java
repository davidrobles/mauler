...
public final class SpeedTest {
     ...
     */
    public static <GAME extends Game<GAME>> double playerSpeed(
            GAME game, Strategy<GAME> strategy, int timeout) {
        Stopwatch stopwatch = Stopwatch.createStarted();
        int nGames = 0;

        while (stopwatch.elapsed(TimeUnit.SECONDS) < timeout) {
            game.reset();
            while (!game.isOver()) game.makeMove(strategy.move(game));
            nGames++;
        }
    ...
    public static double gameSpeed(Game<?> game, int timeout) {
        ...
        while (stopwatch.elapsed(TimeUnit.SECONDS) < timeout) {
            game.reset();
            while (!game.isOver()) game.makeMove(rand.nextInt(game.getNumMoves()));
            nGames++;
        }
