 ...
 * alternates who starts each game to neutralize first-move advantage. Results are aggregated into a
 * win-rate matrix: {@code averages[i][j]} is player {@code i}'s win rate against player {@code j}.
 * The last column ({@code averages[i][n]}) holds player {@code i}'s overall average win rate.
 *
 * <p>Results can be exported as a formatted text table ({@link #toFormattedTable()}), LaTeX ({@link
 * #toLatexTable()}), or CSV ({@link #toCSV()}).
 *
...
public class RoundRobin<GAME extends Game<GAME>> {
     ...
     */
    public RoundRobin(
            Supplier<GAME> gameFactory, int nGames, List<Strategy<GAME>> players, int timeout) {
    ...
    public void run() {
        ...
        for (Series<GAME> s : allSeries) s.run();

        for (Series<GAME> s : allSeries) recordResults(s);
    ...
    public String toFormattedTable() {
        ...
        for (String name : playerNames) sb.append(String.format("%" + col + "s", name));
        ...
        for (int p1 = 0; p1 < players.size(); p1++) {
            ...
            for (int p2 = 0; p2 <= players.size(); p2++) {
                if (p1 == p2) sb.append(String.format("%" + col + "s", "--"));
                else
                    sb.append(
                            String.format(
                                    "%" + col + "s",
                                    String.format(
                                            "%6.1f (%.1f)",
                                            averages[p1][p2] * 100, stdErrors[p1][p2] * 100)));
            }
            sb.append("\n");
        }
    ...
    public String toLatexTable() {
        ...
        for (int i = 0; i <= players.size(); i++) sb.append("r");

        sb.append("}\n\\toprule\nPlayer");

        for (String name : playerNames) sb.append(" & ").append(name);
        ...
        if (!caption.isEmpty()) sb.append("\\caption{").append(caption).append("}\n");
        sb.append("\\end{table*}\n");
    ...
    public String toCSV() {
        ...
        for (int p1 = 0; p1 < players.size(); p1++) {
            ...
            for (int p2 = 0; p2 <= players.size(); p2++) {
                if (p1 == p2) sb.append(",-");
                else sb.append(String.format(",%.6f", averages[p1][p2]));
            }
            sb.append("\n");
        }
    ...
    private void calculateAverages() {
        ...
        for (int p1 = 0; p1 < n; p1++) {
            double total = 0;
            for (int p2 = 0; p2 < n; p2++) if (p1 != p2) total += averages[p1][p2];
    ...
    private int maxPlayerNameLength() {
        int max = 0;
        for (String name : playerNames) if (name.length() > max) max = name.length();
        return max;
    }

    private static <GAME extends Game<GAME>> List<String> defaultNames(
            List<Strategy<GAME>> players) {
        List<String> names = new ArrayList<>();
        for (Strategy<GAME> p : players) names.add(p.toString());
        return names;
    }
