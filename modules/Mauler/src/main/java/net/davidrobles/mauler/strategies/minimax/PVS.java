 ...
 *
 * <p>PVS is a refinement of alpha-beta that exploits the assumption that the first child is the
 * best move (the <em>principal variation</em>):
 *
 * <ol>
 *   <li>The first child is searched with the full {@code [alpha, beta]} window.
 *   <li>Every subsequent child is probed with a zero-width <em>null window</em> {@code [alpha,
 *       alpha+1]}. A null-window search is fast because it cuts off as soon as it finds any move
 *       that beats alpha.
 *   <li>If the probe raises alpha (scout fails high, {@code score > alpha}), the child is
 *       re-searched with the full window to get the exact value.
 * </ol>
 *
 * <p>When move ordering is good the vast majority of nodes require only the cheap null-window
 * probe, making PVS strictly faster than plain alpha-beta in practice while producing identical
 * results.
 *
 * <p>The {@link Evaluator} must return scores from the perspective of the player to move (positive
 * = good, negative = bad).
 *
