 ...
 *
 * <p>Only evaluates terminal positions ({@link Game#isOver()} must be {@code true}); calling {@link
 * #evaluate} on a non-terminal state throws {@link IllegalStateException}.
 *
...
public class TerminalEvaluator<GAME extends Game<GAME>> implements Evaluator<GAME> {
    ...
    /**
     * Creates a terminal evaluator with the standard minimax scores: win = +1.0, loss = -1.0, draw
     * = 0.0.
     */
    ...
    /**
     * Returns an evaluator using MCTS scores: win=1, loss=0, draw=0.5. Scores accumulate as
     * fractions in [0, 1].
     */
    ...
    public double evaluate(GAME game, int player) {
        ...
        switch (result) {
            ...
            case DRAW:
                return draw;
            default:
                throw new IllegalStateException("Unknown GameResult: " + result);
        }
    }
