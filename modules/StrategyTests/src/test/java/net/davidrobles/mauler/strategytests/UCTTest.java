 ...
 *
 * <p>UCT is stochastic, so exact win counts cannot be asserted. The threshold is conservative —
 * well below the true win rate — to avoid flakiness while still catching regressions. Tests are run
 * as both player 0 and player 1.
 */
public class UCTTest {
    ...
    private static void assertWinsOverwhelmingly(int player) {
        ...
        assertTrue(
                String.format(
                        "UCT(%d) win rate as player %d: %.1f%% < %.0f%% threshold",
