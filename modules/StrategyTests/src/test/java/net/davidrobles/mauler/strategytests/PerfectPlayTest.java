 ...
 * must never lose as either player, and perfect play against perfect play
 * must always draw.
 */
public class PerfectPlayTest
{
    ...
    /**
     * Asserts that {@code strategy} never loses against a random opponent
     * in {@code N_GAMES} games as player 0, then again as player 1.
     */
    private static void assertNeverLoses(Strategy<TicTacToe> strategy, String name)
    {
        Series<TicTacToe> asP0 = new Series<>(TicTacToe::new, N_GAMES,
                List.of(strategy, new RandomStrategy<>()));
        asP0.setVerbose(false);
        asP0.run();
        assertEquals(name + " lost as player 0", 0, asP0.getLosses(0));

        Series<TicTacToe> asP1 = new Series<>(TicTacToe::new, N_GAMES,
                List.of(new RandomStrategy<>(), strategy));
