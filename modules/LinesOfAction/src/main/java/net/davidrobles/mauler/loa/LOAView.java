...
public class LOAView extends JPanel
        implements MoveObserver, MatchControllerObserver<LOA>, MouseListener {
    private LOA loa;
        ...
        for (int row = 0; row < LOA.SIDE_SIZE; row++) {
            for (int col = 0; col < LOA.SIDE_SIZE; col++) {
                ...
                if (loa.getCell(row, col) != LOA.Cell.EMPTY) {
                    if (loa.getCell(row, col) == LOA.Cell.BLACK) g.setPaint(P1_STONE_COLOR);
                    else if (loa.getCell(row, col) == LOA.Cell.WHITE) g.setPaint(P2_STONE_COLOR);
    ...
    private void drawMessages(Graphics2D g) {
        if (loa.isOver()) {
            ...
            if (loa.getOutcome().orElseThrow()[0] == GameResult.WIN) msg = "Strategy 1 wins";
            else if (loa.getOutcome().orElseThrow()[1] == GameResult.WIN) msg = "Strategy 2 wins";
            else msg = "Draw!";
    ...
    private void drawLegalMoves(Graphics2D g) {
        if (row != -1 && col != -1) {
            ...
            for (String moveStr : loa.filterMoves(row, col)) {
                ...
                g.fillOval(
                        dstPoint.x * CELL_SIZE + BOARD_X_OFFSET + CELL_X_OFFSET,
    ...
    private int getCol(char letter) {
        for (int i = 0; i < LOA.SIDE_SIZE; i++) if ((char) ('a' + (i)) == letter) return i;
