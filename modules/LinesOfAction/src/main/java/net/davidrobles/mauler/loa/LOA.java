...
public class LOA extends ObservableGame<LOA> {
    ...
    public Cell[] getBoard() {
        ...
        for (int i = 0; i < N_CELLS; i++) board[i] = getCell(i);
        return board;
    }

    /**
     * Returns the type of the cell for the given cell index. The index goes from 0 to 63. 0
     * representing the top left cell, and 63 the bottom right cell.
     *
     ...
     * with 0 being the top row and 7 the bottom row. Columns order goes from left to right, with 0
     * being the first column and 7 being the last column.
     *
    ...
    /**
     * Returns a bitboard with the pieces of the player in turn. For example, if the player to move
     * is 0 (X's), it will return a bitboard (64 bit long), with 1's where there are black pieces,
     * and 0's where there are either empty cells or white pieces.<br>
     * <br>
    ...
    /**
     * Returns a bitboard with the pieces of the player NOT in turn. For example, if the player to
     * move is 0 (X's), it will return a bitboard (64 bit long), with 1's where there are white
     * pieces, and 0's where there are either empty cells or black pieces.<br>
     * <br>
     * The following board:<br>
     * <br>
     * - X - X - - - - <br>
    ...
    /**
     * Sets the bitboard of the player in turn. It must be a long representing all the pieces of the
     * player in turn.
     *
     * @param bitboard the new bitboard
     */
    private void setCurrentBitboard(long bitboard) {
        if (current == 0) black = bitboard;
        else white = bitboard;
    }

    /**
     * Sets the bitboard of the player NOT in turn. It must be a long representing all the pieces of
     * the player NOT in turn.
     *
     * @param bitboard the new bitboard
     */
    private void setOpponentBitboard(long bitboard) {
        if (current == 0) white = bitboard;
        else black = bitboard;
    }
        ...
        for (int i = 0; i < N_CELLS; i++) {
            ...
            if (cur != 0L) {
                ...
                if (candidateMove != 0L && ((candidateMove & curBitboard) == 0L)) {
                    ...
                    if (test) moves.add(new long[] {cur, candidateMove});
                }
    ...
    private void printEntry(Map.Entry<Long, List<Long>> entry) {
        ...
        for (Long move : entry.getValue()) legal |= move;

        StringBuilder builder = new StringBuilder();

        builder.append("   ");

        for (int i = 0; i < SIDE_SIZE; i++) builder.append((" " + ((char) ('A' + i)) + " "));

        builder.append("\n");

        for (int i = 0; i < N_CELLS; i++) {
            if (i % SIDE_SIZE == 0) builder.append((" " + (i / SIDE_SIZE + 1) + " "));

            if ((current & (1L << i)) != 0) builder.append(" \u25C9 ");
            else if ((legal & (1L << i)) != 0) builder.append(" x ");
            else builder.append(" - ");

            if (i % SIDE_SIZE == SIDE_SIZE - 1) builder.append((" " + (i / SIDE_SIZE + 1) + " \n"));
        }
    ...
    /**
     * Returns the index of the given move. The move is given in a bitboard representation.<br>
     * <br>
     * Example:<br>
     * <br>
     * if the bitboard is:<br>
     * <br>
     * - - - - - - - - <br>
     * - X - - - - - - <br>
     * - - - - - - - - <br>
     ...
     * - - - - - - - - <br>
     * <br>
     * 00000000 00000000 00000000 00000000 00000000 00000000 00000010 00000000<br>
     * <br>
    ...
    private void floodFill(long bitboard, long node, long visited) {
        if (((node & visited) != 0L) || (bitboard & node) == 0L) return;
    ...
    /**
     * Returns a python-style string representation of the array of all the legal moves.<br>
     * <br>
     * For example: <br>
     * <br>
     * [a5-e3, b3-c5, d6-a2]
    ...
    private String getMovesString() {
        ...
        for (String str : getMoves()) builder.append((str + ", "));
    ...
    /**
     * Returns the coordinates of the source cell of the given move.<br>
     * <br>
     * Example: <br>
     * <br>
     * "a8-c8" => (row: 0, col: 0)<br>
     * "a5-g5" => (row: 3, col: 0)<br>
     * "e3-d5" => (row: 5, col: 4)<br>
     * <br>
     *
    ...
    /**
     * Returns the coordinates of the target cell of the given move.<br>
     * <br>
     * Example: <br>
     * <br>
     * "a8-c8" => (row: 0, col: 2)<br>
     * "a5-g5" => (row: 3, col: 6)<br>
     * "e3-d5" => (row: 3, col: 3)<br>
     * <br>
     *
     * @param moveStr a string representation of a move
    ...
    /**
     * Returns the index of the column represented by the given character.<br>
     * <br>
     * Example:<br>
     * <br>
     * 'a' => 0<br>
    ...
    private int getCol(char letter) {
        for (int i = 0; i < SIDE_SIZE; i++) if ((char) ('a' + (i)) == letter) return i;

        return -838448;
    }

    /**
     * Returns a collection of the legal moves for the given cell. For example: filterMoves(3, 2)
     * could return a Collection ["c5-c3", "c5-c7", "c5-a5"]
     *
     * @param row the row of the cell
    ...
    public Collection<String> filterMoves(int row, int col) {
        ...
        for (String move : getMoves())
            if (cellToString(row, col).equals(move.substring(0, 2))) newMoves.add(move);
    ...
    public LOA copy() {
        ...
        for (long[] move : moves) newLOA.moves.add(new long[] {move[0], move[1]});
    ...
    public List<String> getMoves() {
        ...
        for (long[] move : moves)
            legalMoves.add(
                    cellToString(cellBitToIndex(move[0]))
                            + "-"
                            + cellToString(cellBitToIndex(move[1])));
    ...
    public String toString() {
        ...
        for (int i = 0; i < N_CELLS; i++) {
            if (i % SIDE_SIZE == 0) builder.append((" " + (8 - (i / SIDE_SIZE)) + " "));
    ...
    /**
     * Resets the state of the game. It resets both bitboards: black and white.<br>
     * <br>
     * Black bitboard to:<br>
     * <br>
     * - X X X X X X - <br>
     * - - - - - - - - <br>
     * - - - - - - - - <br>
     ...
     * - X X X X X X - <br>
     * <br>
     * 01111110 00000000 00000000 00000000 00000000 00000000 00000000 01111110<br>
     * <br>
     * and the white bitboard to:<br>
     * <br>
     * - - - - - - - - <br>
     * 0 - - - - - - 0 <br>
     * 0 - - - - - - 0 <br>
     ...
     * - - - - - - - - <br>
     * <br>
     * 00000000 10000001 10000001 10000001 10000001 10000001 10000001 00000000<br>
     * <br>
     */
    public void setInitialBoard() {
        int[] blackInit = new int[] {1, 2, 3, 4, 5, 6, 57, 58, 59, 60, 61, 62};

        for (int blackIndex : blackInit) black |= (1L << blackIndex);

        int[] whiteInit = new int[] {8, 16, 24, 32, 40, 48, 15, 23, 31, 39, 47, 55};

        for (int whiteIndex : whiteInit) white |= (1L << whiteIndex);
    }
    ...
    /**
     * Converts the given cell to a String representation.<br>
     * <br>
     * Examples:<br>
     * <br>
     * 0 => 'a8'<br>
    ...
    public static String cellToString(int cellIndex) {
        return String.format(
                "%c%d", (char) ('a' + cellIndex % SIDE_SIZE), (8 - (cellIndex / SIDE_SIZE)));
    }

    /**
     * Converts the coordinates of a cell to a String representation.<br>
     * <br>
     * Examples:<br>
     * <br>
     * (0, 0) => 'a8'<br>
    ...
    /**
     * Converts the coordinates of a cell of the board to an index representing the same cell in the
     * board.
     *
     * @param row the row of the cell
     * @param col the column of the cell
    ...
    public static void printBitBoard(Long bitboard) {
        for (long i = 0; i < N_CELLS; i++) {
            if ((bitboard & (1L << i)) != 0) System.out.print(" X ");
            else System.out.print(" - ");

            if (i % SIDE_SIZE == SIDE_SIZE - 1) System.out.println();
        }
